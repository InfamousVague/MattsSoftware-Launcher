...
/// All the native side-effects, ported 1:1 from the launcher's
/// Rust (`catalog.rs` / `install.rs`): installed-version probe,
/// GitHub latest-release lookup, and the DMG install pipeline
/// (download → hdiutil attach → ditto → detach). Pure
/// Foundation/AppKit, no third-party anything.
enum Services {
    ...
    /// version-unknown (mirrors the Rust contract).
    ...
    /// Update (same policy as the Rust side; avoids mis-ranking a
    /// non-semver tag into a false up-to-date).
